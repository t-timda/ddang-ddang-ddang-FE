import React from "react";
import OngoingTrialItem from "./OngoingTrialItem";

interface OngoingCase {
  caseId: number;
  title: string;
  argumentAMain: string;
  argumentBMain: string;
  authorNickname: string;
  rivalNickname: string;
  status: string;
  createdAt: string;
}

interface OngoingTrialTableProps {
  cases: OngoingCase[];
  startIndex: number;
  totalCount: number;
  onCaseClick: (caseId: number) => void;
}

const OngoingTrialTable: React.FC<OngoingTrialTableProps> = ({
  cases,
  startIndex,
  totalCount,
  onCaseClick,
}) => {
  return (
    <div className="bg-white rounded-lg overflow-hidden border-2 border-gray-300">
      {/* 테이블 헤더 */}
      <div className="grid grid-cols-12 gap-4 bg-main-bright px-6 py-4 font-bold text-main-medium">
        <div className="col-span-1 text-center border-r border-gray-300">순서</div>
        <div className="col-span-4 text-center border-r border-gray-300">주제</div>
        <div className="col-span-3 text-center border-r border-gray-300">A 주장</div>
        <div className="col-span-3 text-center border-r border-gray-300">B 주장</div>
        <div className="col-span-1 text-center">참여 변호사 현황</div>
      </div>

      {/* 테이블 바디 */}
      {cases.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-gray-500 text-lg">진행 중인 재판이 없습니다.</p>
        </div>
      ) : (
        <div className="divide-y divide-gray-200">
          {cases.map((caseItem, idx) => (
            <OngoingTrialItem
              key={caseItem.caseId}
              index={totalCount - (startIndex + idx)}
              caseId={caseItem.caseId}
              title={caseItem.title}
              argumentAMain={caseItem.argumentAMain}
              argumentBMain={caseItem.argumentBMain}
              status={caseItem.status}
              onClick={() => onCaseClick(caseItem.caseId)}
            />
          ))}
        </div>
      )}
    </div>
  );
};

export default OngoingTrialTable;

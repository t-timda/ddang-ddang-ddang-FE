...
import { useUpdateUserProfileMutation, useUploadProfileImageMutation } from "@/hooks/api/useUserQuery";
...
const MyPage = () => {
  ...
  const initialSubTab = searchParams.get('subtab') as "전체" | "진행중" | "변호전적" | null;
  ...
  const [participateTab, setParticipateTab] = useState<"전체" | "진행중" | "변호전적">(initialSubTab || "전체");
  ...
  useEffect(() => {
    ...
    if (subtab && (subtab === "전체" || subtab === "진행중" || subtab === "변호전적")) {
      setParticipateTab(subtab);
    }
  }, [searchParams]);

  // 필터링 훅 사용
  const { filteredAllItems, filteredOngoingTrials, filteredDefenseList } = useMyPageFilters(
    allItems,
    ongoingTrialsWithType,
    defenseListWithResult,
    sortType,
  ...
  const achievementTotalPages = Math.ceil((achievementsData?.result?.length ?? 0) / ACHIEVEMENTS_PER_PAGE);
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-4 flex flex-col">
        ...
        <div className="flex flex-col lg:flex-row w-full gap-6 lg:gap-0">
          ...
          <div className="w-full lg:w-3/4 lg:pl-8 lg:pt-8">
            ...
            {selectedMenu === "exp" && (
              <ExpTab currentRank={currentRank} currentExp={currentExp} nickname={nickname} />
            )}
